  ...
  game.audioSetMusic(true);
  {
    ...
    if (rc === 0) {
      ...
      lastStageNote = game.gdlRaw()
        ...
        : game.gdlC0()
        ? `Inflated 1172 C0 + vtx + player look-at + G_SETTEX (IA/RGBA32 + per-tri cache + secondary GDL + near-plane clip). last_draw=${game.lastDraw()} rooms=${game.bgRooms()} gdlC0=1 vtx=${game.gdlVtx() ? 1 : 0}. After 0b83df6 some wall tiles already bound; floors/ceilings that were black should sample if their SETTEX id is in the pack (HUD texOk/texMiss are honest). Keys 1-4 split-screen (ENV ${game.envPlayers()}). P1 WASD+Z, P2 arrows+Enter.`
