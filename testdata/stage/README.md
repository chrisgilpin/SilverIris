...
an END-only setup must keep the greyscale G1 hash. Far pads cull. Door-sized bg portals (and the start alcove) with no nearby pad door get the retail 96-vert door mesh when P*Z G_VTX is segment 4 (bind file+0xC0; never seg 3). Synthetic packs keep the G1DL metal quad (SETTEX 685).
