...
/* Oliveguard leftover SETTEX 1967 (32x48 camo tile, not 1916/1968). Baked
 * cn=80 off no_mtx must keep texel albedo. */
static int test_olive_tex_albedo(void)
{
    uint8_t vtx_be[48];
    static Vtx vtx[3];
    int i, r, c;
    ...
    g1_tex_unload();
    if (g1_tex_load_raw(1967, G1_TEX_I8, 8, 8, white, 64, NULL, 0) != 0)
        return fail("olive tex albedo load");
    for (r = 0; r < 4; r++)
        for (c = 0; c < 4; c++)
            id[r][c] = (r == c) ? 1.f : 0.f;
    g0_mtx_f2l(id, &mv);
    ...
    g1_set_shade_modulate(1);
    if (g1_interpret_be_dl(gdl, ngfx) != 0)
        return fail("olive tex albedo interpret");
    fb = g1_fb_rgba();
    n = (unsigned)G1_FB_W * (unsigned)G1_FB_H;
    for (k = 0; k < n; k++) {
        unsigned pr = fb[k * 4];
    ...
    if (full < 1000)
        return fail("olive SETTEX 1967 must keep albedo off no_mtx");
    if (mid > 80)
        return fail("olive SETTEX 1967 still SHADE-flattened");
    g1_tex_unload();
    printf("olive tex albedo full=%u (cn80 ignored, SETTEX 1967)\n", full);
    return 0;
}
